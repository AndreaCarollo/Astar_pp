...
Interface::Interface(ConfigReader *p)
{
    ...
    intersection_map = cv::Mat(interface_size, CV_8U, cv::Scalar());
...
void Interface::update(PathPlanning *plan)
{
    target = cv::Point(plan->target.col * scale,
                       plan->target.row * scale);

    if (plan->distance_robot_target >= plan->distance_threshold)
    {
        if (plan->path_planning)
            put_path(plan->grid, plan->target);
        else
            put_arrow();
    }
    else
...
void Interface::put_simplified_path(std::vector<cv::Point2i> path_list_point)
{
    intersection_map_path = cv::Mat(interface_size, CV_8UC1, cv::Scalar(0));
    // Starting from the target point follow the path until
    // the starting point where the come_from pointer is NULL
    for (int i = 0; i < path_list_point.size() - 1; i++)
    {
        cv::line(intersection_map_path, path_list_point[i], path_list_point[i + 1], cv::Scalar(50), scale / 2);
    }
    cv::add(intersection_map_path,intersection_map,intersection_map_path);
}

void Interface::put_simplified_path(cv::Point2i ptA, cv::Point2i ptB)
{
    intersection_map_path = cv::Mat(interface_size, CV_8UC1, cv::Scalar(0));
    // Starting from the target point follow the path until
    // the starting point where the come_from pointer is NULL
